...
`socket`으로 구글 공개 DNS(8.8.8.8)에 UDP 연결을 시도합니다. UDP는 실제 연결을 맺지 않으므로 패킷이 나가지 않고, OS가 "외부로 나갈 때 어떤 내 IP를 쓸지"만 계산해 줍니다. 그 IP를 추출해 내 주소를 얻습니다.

이어서 `netifaces`로 서브넷 마스크를 가져옵니다. 게이트웨이가 사용하는 인터페이스(IPv4 = key 2)를 찾고, 그 네트워크 카드의 주소 정보에서 넷마스크를 추출합니다. IP와 넷마스크를 `ipaddress`로 합쳐 스캔할 대역(예: `192.168.0.0/24`)을 만듭니다.
